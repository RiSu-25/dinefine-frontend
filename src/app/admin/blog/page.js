...
const resolveImage = (url) => {
  ...
  if (url.startsWith("/")) return `${API_BASE}${url}`; // /uploads/file.jpg
  return `${API_BASE}/${url}`;
};
...
export default function BlogPage() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const normalized = data.map((b) => ({
        ...b,
        image: resolveImage(b.imageUrl), // 🔥 Corrected image handling
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/blog/${blogToDelete.id}`, {
        method: "DELETE",
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#E4F1ED] to-[#D8E8F2] p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="bg-[#009BA8] hover:bg-[#007C88] text-white px-5 py-2 rounded-lg shadow-md transition"
        >
          + Add Blog
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          blogs.map((blog) => (
            <div
              key={blog.id}
              className="backdrop-blur-md bg-white/40 rounded-xl shadow-md overflow-hidden border border-white/30"
            >
              ...
              <div className="p-4">
                <p className="text-sm text-[#009BA8] font-medium mb-1">
                  {blog.date}
                </p>
                <h2 className="text-lg font-semibold text-[#117D85] mb-2">
                  {blog.title}
                </h2>
                ...
                <div className="flex justify-end gap-3 mt-4">
                  ...
                  <button
                    ...
                    className="text-[#009BA8] hover:text-[#007C88]"
                    title="Edit"
                  >
                    <FaEdit />
                  </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="backdrop-blur-md bg-white/70 border border-white/30 rounded-xl p-6 w-full max-w-2xl shadow-xl">
            <h2 className="text-xl font-semibold text-[#117D85] mb-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                <label className="text-sm text-[#117D85] font-medium">
                  Upload Image
                </label>
                <input
                  type="file"
              ...
              <div className="flex justify-end gap-3 mt-4">
                ...
                <button
                  onClick={handleSave}
                  className="px-4 py-2 bg-[#009BA8] text-white rounded-lg cursor-pointer"
                >
      ...
      {selectedBlog && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="backdrop-blur-md bg-white/80 rounded-xl border border-white/30 p-6 w-full max-w-2xl relative">
            <button
              className="absolute top-3 right-3 text-gray-600 hover:text-[#009BA8]"
            ...
            <h2 className="text-xl font-bold text-[#117D85] mt-1 mb-2">
              {selectedBlog.title}
            </h2>
      ...
      {isDeleteModalOpen && blogToDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="backdrop-blur-md bg-white/70 border border-white/30 rounded-xl p-6 w-full max-w-sm text-center shadow-xl">
            <h2 className="text-lg font-semibold text-[#117D85] mb-4">
              Delete Blog
            </h2>

            <p className="text-gray-700 mb-6">
              Are you sure you want to delete{" "}
              <span className="font-medium text-[#009BA8]">
                “{blogToDelete.title}”
              </span>
              ?
            </p>
