...
/* 🔥 FIXED: Handles Cloudinary + local + missing image */
const getImageUrl = (img) => {
  if (!img) return "/blog/blog1.jpg"; // fallback
    ...
    const fetchBlogs = async () => {
      ...
      try {
        ...
        const normalized = data.map((item, idx) => {
          ...
          const description =
            full.length > 150 ? full.slice(0, 150).trim() + "..." : full;
    ...
    <section className="bg-[#fff8f1] min-h-screen font-poppins">
      ...
      <div className="relative h-[50vh] md:h-[60vh] flex items-center justify-center text-center">
        ...
        <div className="relative z-10 px-6">
          ...
          <p className="text-[#e7a738] text-sm md:text-lg max-w-2xl mx-auto drop-shadow-md">
            Delicious stories, creative recipes, and the people behind DineFine’s
            most-loved dishes.
          </p>
        </div>
      </div>
      ...
      <div className="pt-20 pb-20 px-6 md:px-12 lg:px-24">
        <div className="text-center mb-14">
          <h2 className="text-3xl md:text-4xl font-bold text-[#4b3b23] mb-3">
            Latest from the Kitchen
          </h2>
          <p className="text-[#6e5a3a] max-w-2xl mx-auto text-sm md:text-base">
            Explore our newest updates — from behind-the-scenes insights to
            seasonal specialties and kitchen experiments.
          </p>
        </div>
                  ...
                  <div className="p-5 flex flex-col justify-between h-[250px]">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        {Icon ? (
                          <Icon className="text-[#c28b2f] text-xl" />
                        ) : null}
                      ...
                      <h3 className="text-lg md:text-xl font-semibold text-[#4b3b23] mb-2 line-clamp-2">
                        {post.title}
                      </h3>

                      <p className="text-[#6e5a3a] text-sm line-clamp-3">
                        {post.description}
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="mt-4 text-[#b89038] font-semibold hover:underline"
                    >
                      Read More →
                    </button>
                  </div>
